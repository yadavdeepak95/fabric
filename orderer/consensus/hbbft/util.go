...
//Peer For hbbft to extract ids used my implementation
type Peer struct {
	ID      uint64
	Context []byte
}

// Peers maps consenters to slice of raft.Peer
func Peers(consenterIDs []uint64) []Peer {
	var peers []Peer

	for _, hbbftID := range consenterIDs {
		peers = append(peers, Peer{ID: hbbftID})
	}
	return peers
}
