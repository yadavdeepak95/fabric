...
//Config TODO : if required add block size here
...
func (c *Consenter) ReceiverByChain(channelID string) MessageReceiver {
	...
	if hbbftChain, isHbbftChain := cs.Chain.(*Chain); isHbbftChain {
		return hbbftChain
	}
	c.Logger.Warningf("Chain %s is of type %v and not hbbftChain.Chain", channelID, reflect.TypeOf(cs.Chain))
	return nil
}

//TODO need to change to hbbft i don't think so as of now
func (c *Consenter) detectSelfID(consenters map[uint64]*etcdtohbbft.Consenter) (uint64, error) {
...
//HandleChain TODO option can by used as diifferent names
// HandleChain returns a new Chain instance or an error upon failure
func (c *Consenter) HandleChain(support consensus.ConsenterSupport, metadata *common.Metadata) (consensus.Chain, error) {
	m := &etcdtohbbft.ConfigMetadata{}
	if err := proto.Unmarshal(support.SharedConfig().ConsensusMetadata(), m); err != nil {
		return nil, errors.Wrap(err, "failed to unmarshal consensus metadata")
	}

	if m.Options == nil {
		return nil, errors.New("hbbft options have not been provided")
	}
	...
	if err != nil {
		fmt.Printf("*****************************CALLING TRACK CHAIN")
...
// otherwise, it reads hbbft metadata from config metadata supplied.
func ReadBlockMetadata(blockMetadata *common.Metadata, configMetadata *etcdtohbbft.ConfigMetadata) (*etcdtohbbft.BlockMetadata, error) {
	if blockMetadata != nil && len(blockMetadata.Value) != 0 { // we have consenters mapping from block
		m := &etcdtohbbft.BlockMetadata{}
...
func New(
	...
	logger := flogging.MustGetLogger("orderer.consensus.hbbft")
	...
	consenter.Logger.Infof("HBBFT consenter")
	return consenter
}
