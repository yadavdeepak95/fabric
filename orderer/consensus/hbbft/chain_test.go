...
import (
	...
	etcdraft "github.com/hyperledger/fabric/orderer/consensus/hbbft"
	"github.com/hyperledger/fabric/orderer/consensus/hbbft/mocks"
...
var _ = Describe("Chain", func() {
	...
	Describe("Single Raft node", func() {
		...
		JustBeforeEach(func() {
			chain, err = etcdraft.NewChain(support, opts, configurator, nil, cryptoProvider, noOpBlockPuller, nil, nil)
			Expect(err).NotTo(HaveOccurred())
	...
	Describe("4-node hbbft cluster", func() {
		var (
			network        *network
			channelID      string
		...
		When("2/3 nodes are running", func() {
			It("late node can catch up", func() {
				...
				// Eventually(func() int { return c0.support.WriteBlockCallCount() }, LongEventualTimeout).Should(Equal(1))
				...
				// err = c3.Order(env, 1)
				Expect(err).NotTo(HaveOccurred())
				Eventually(func() int { return c0.support.WriteBlockCallCount() }, LongEventualTimeout).Should(Equal(2))
				...
				env = &common.Envelope{
					Payload: marshalOrPanic(&common.Payload{
						Header: &common.Header{ChannelHeader: marshalOrPanic(&common.ChannelHeader{Type: int32(common.HeaderType_MESSAGE), ChannelId: channelID})},
						Data:   []byte("TEST_MESSAGE2   222333"),
					}),
				}
				err = c0.Order(env, 2)
				Expect(err).NotTo(HaveOccurred())
				...
				env = &common.Envelope{
					Payload: marshalOrPanic(&common.Payload{
						Header: &common.Header{ChannelHeader: marshalOrPanic(&common.ChannelHeader{Type: int32(common.HeaderType_MESSAGE), ChannelId: channelID})},
						Data:   []byte("TEST_MESSAGE2   333222333"),
					}),
				}
				err = c0.Order(env, 2)
				Expect(err).NotTo(HaveOccurred())
				...
				// err = c3.Order(env, 2)
				// Expect(err).NotTo(HaveOccurred())
				Eventually(func() int { return c0.support.WriteBlockCallCount() }, LongEventualTimeout).Should(Equal(4))
				Eventually(func() int { return c1.support.WriteBlockCallCount() }, LongEventualTimeout).Should(Equal(4))
				Eventually(func() int { return c2.support.WriteBlockCallCount() }, LongEventualTimeout).Should(Equal(4))
				Eventually(func() int { return c3.support.WriteBlockCallCount() }, LongEventualTimeout).Should(Equal(4))
		...
		When("reconfiguring raft cluster", func() {
			...
			Context("channel creation", func() {
				It("succeeds with valid config metadata", func() {
					...
					Expect(c0.Configure(createChannelEnv(metadata), 0)).To(Succeed())
					// Expect(c1.Configure(createChannelEnv(metadata), 0)).To(Succeed())
					// Expect(c2.Configure(createChannelEnv(metadata), 0)).To(Succeed())
					// Expect(c3.Configure(createChannelEnv(metadata), 0)).To(Succeed())
...
// 			Expect(err).NotTo(HaveOccurred())It("stale leader should not be able to propose block because of lagged term", func() {
// 				network.disconnect(1)
// 				network.elect(2)
// 				network.connect(1)
...
func newChain(
	...
	// c.Node = honeybadgerbft.NewWrapper(1, int(c.id), len(c.opts.Consenters))
