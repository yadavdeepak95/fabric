...
// 					indices := hbbft.ListSnapshots(logger, c2.opts.SnapDir)
...
func (c *chain) init() {
	ch, err := hbbft.NewChain(
		c.support,
		c.opts,
		c.configurator,
		c.rpc,
		func() (hbbft.BlockPuller, error) { return c.puller, nil },
		nil,
		nil,
	)
	Expect(err).NotTo(HaveOccurred())
...
func noOpBlockPuller() (hbbft.BlockPuller, error) {
